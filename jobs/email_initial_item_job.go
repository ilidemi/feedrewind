...
func init() {
	registerJobNameFunc("EmailInitialItemJob", func(ctx context.Context, conn *pgw.Conn, args []any) error {
		if len(args) != 3 {
...
func EmailInitialItemJob_Perform(
	...
	return util.Tx(conn, func(tx *pgw.Tx, conn util.Clobber) error {
		...
		row := tx.QueryRow(`
			select name, initial_item_publish_status, (select email from users where id = $1)
