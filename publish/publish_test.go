...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/">
  <channel>
	...
	conn, err := db.Pool.Acquire(context.Background()) //nolint:gocritic,staticcheck
...
func TestIsPausedHandling(t *testing.T) {
	...
	_, err = conn.Exec(`update subscriptions set is_paused = true where id = $1`, subscription.Id)
	oops.RequireNoError(t, err)
...
func cleanup(tx pgw.Queryable) error {
	...
	tables := []string{
		"subscription_posts", "blog_posts", "schedules", "subscriptions", "blogs", "user_settings", "users",
	}
